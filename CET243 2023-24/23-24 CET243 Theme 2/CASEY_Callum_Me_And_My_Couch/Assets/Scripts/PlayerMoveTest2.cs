...
public class PlayerMoveTest2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _inputActionReference.Player1.Movement.performed += moving =>
        {
            _playersMovementDirection = moving.ReadValue<float>();
        };

        _inputActionReference.Player1.Jump.performed += jumping =>
        {
            JumpThePlayer();
        };

        _inputActionReference.Player1.Quit.performed += Quit =>
        {
            Application.Quit();
        };
    }

...
public class PlayerController2 : MonoBehaviour, IPlayerController
{
    ...
    private void Update()
    {
        ...
        CalculateJumpApex(); // Affects fall speed, calculate before gravity
    ...
    private void CalculateWalk()
    {
        ...
        if (_currentHorizontalSpeed > 0 && facingRight == false)
        {
            transform.Rotate(0f, 180f, 0f);
            facingRight = true;
        }
        else if (_currentHorizontalSpeed < 0 && facingRight == true)
        {
            transform.Rotate(0f, 180f, 0f);
            facingRight = false;
        }
    ...
    // Cast bounds before moving to avoid future collisions
    private void MoveCharacter()
    {
        ...
        for (int i = 1; i < _freeColliderIterations; i++)
        {
            ...
            if (Physics2D.OverlapBox(posToTry, _characterBounds.size, 0, _groundLayer))
            {
                ...
                // Landed on a corner or hit our head on a ledge. Nudge the player gently
                if (i == 1)
                {
                    if (_currentVerticalSpeed < 0) _currentVerticalSpeed = 0;
    ...
    public void DamagePlayer2(int damage)
    {
        ...
        if (health <= 0)
        {
            SpawnPlayer.DestroyPlayer2(this);
        }
    }
}